package com.github.my.config;

import com.github.my.handler.*;
import com.github.my.handler.LogHandler;
...
//        WxMpService wxMpService = new me.chanjar.weixin.mp.api.impl.apache.WxMpServiceImpl();
    ...
    public WxMpMessageRouter router(WxMpService wxMpService) {
        ...
        // 自定义菜单事件
        newRouter.rule().async(false).msgType(XmlMsgType.EVENT)
                .event(MenuButtonType.CLICK.toUpperCase()).handler(this.getMenuHandler()).end();

        // 点击菜单连接事件
        newRouter.rule().async(false).msgType(XmlMsgType.EVENT)
                .event(MenuButtonType.VIEW.toUpperCase()).handler(this.nullHandler).end();
